...
namespace CrystalUnbolt
{
    ...
    {
        #region Extensions
        public static AnimCase WaitForEnd(this Animation tweenObject, float delay = 0, bool unscaledTime = false, UpdateMethod updateMethod = UpdateMethod.Update)
        {
            return new Wait(tweenObject).SetDelay(delay).SetUnscaledMode(unscaledTime).SetUpdateMethod(updateMethod).StartTween();
        }

        public static AnimCase DOLayerWeight(this Animator tweenObject, string layerName, float resultValue, float time, float delay = 0, bool unscaledTime = false, UpdateMethod updateMethod = UpdateMethod.Update)
        {
        ...
        public static AnimCase DOLayerWeight(this Animator tweenObject, int layerID, float resultValue, float time, float delay = 0, bool unscaledTime = false, UpdateMethod updateMethod = UpdateMethod.Update)
        {
