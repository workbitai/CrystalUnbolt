...
{
    public class CrystalMapBehavior : MonoBehaviour
    {
        ...
        public CrystalMapChunkBehavior LowestLoadedChunk => loadedChunks[0];
        public CrystalMapChunkBehavior HighestLoadedChunk => loadedChunks[^1];
        ...
        private AnimCase rubberCase;

        private void Awake()
        {
            instance = this;
            loadedChunks = new List<CrystalMapChunkBehavior>();
            ...
            ScreenManager.PopupOpened += PopupStateChanged;
            ScreenManager.PopupClosed += PopupStateChanged;
        }
        ...
        {
            // Destroy existing chunks immediately and rebuild using current MaxLevelReached
        ...
        private void PopupStateChanged(IPopupWindow popupWindow, bool state)
        {
            isMouseDown = false;
        ...
        public void Show()
        {
            ...
            CrystalMapChunkBehavior lastReachedChunk = Instantiate(data.chunks[lastReachedChunkId % data.chunks.Count]).GetComponent<CrystalMapChunkBehavior>();
            ...
            if (!CrystalGameManager.Data.InfiniteLevels)
            {
                for (int i = 0; i < loadedChunks.Count; i++)
                {
                    CrystalMapChunkBehavior chunk = loadedChunks[i];
        ...
        private int GetLastReachedChunkId(out int totalLevelsCount)
        {
            ...
            while (totalLevelsCount <= MaxLevelReached)
            {
                ...
                CrystalMapChunkBehavior chunk = data.chunks[lastReachedChunkId % data.chunks.Count].GetComponent<CrystalMapChunkBehavior>();
        ...
        private void Update()
        {
            ...
            if (!isMouseDown && CrystalInputHandler.ClickAction.WasPressedThisFrame())
            {
                // mouse press y position mapped on 0-1 scale. 0 is the bottom of the screen, 1 is the top)
                mousePressPosY = CrystalInputHandler.MousePosition.y / Camera.main.pixelHeight;
            ...
            else if (isMouseDown && CrystalInputHandler.ClickAction.WasReleasedThisFrame())
            {
                isMouseDown = false;
                ...
                else
                {
                    ...
                    mouseReleasePosY = CrystalInputHandler.MousePosition.y / Camera.main.pixelHeight;
            ...
            {
                float mousePosY = CrystalInputHandler.MousePosition.y / Camera.main.pixelHeight;
        ...
        private void ContinuousScroll(float scrollFrameDistance)
        {
            ...
            }).SetEasing(Ease.Type.SineOut).OnComplete(() =>
            {
                ...
                else if (!CrystalGameManager.Data.InfiniteLevels)
                {
                    for (int i = 0; i < loadedChunks.Count; i++)
                    {
                        CrystalMapChunkBehavior chunk = loadedChunks[i];
        ...
        private void TopRubber(CrystalMapChunkBehavior chunk)
        {
        ...
        private void CheckBottomChunks()
        {
            ...
            {
                CrystalMapChunkBehavior newLowestChunk = Instantiate(data.chunks[(LowestLoadedChunk.ChunkId - 1) % data.chunks.Count]).GetComponent<CrystalMapChunkBehavior>();
        ...
        private void CheckTopChunks()
        {
            ...
            {
                CrystalMapChunkBehavior newHighestChunk = Instantiate(data.chunks[(HighestLoadedChunk.ChunkId + 1) % data.chunks.Count]).GetComponent<CrystalMapChunkBehavior>();
